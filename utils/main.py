...
def create_replacer(data:pd.DataFrame,row:int):
    ...
    for old in replacer_.copy().keys():
...
def multi_report_n(data:pd.DataFrame,doc:docx.document.Document,main_name:str,folder_name:str,
                   feature_list:list,column_namer:str): # 
    
    nro_report = data.shape[0]
    # new_doc = copy.copy(doc)
    if main_name == "":main_name = "document"
    if "Personalizar Nombre de Documento." in feature_list:
        
        for nro in range(0,nro_report):
            new_doc = keyword_replacer_document(doc,create_replacer(data,row = nro))

            try:
                new_doc.save('data/output/{path_id_user}/{name}_{custom_name}.docx'.format(path_id_user=folder_name,name=main_name,custom_name=data.loc[nro][column_namer]))
                print("FLUJO 1")
            ...
            #     # print(replacer_dict)
            #     # doc = docx.Document("in_word/Informe Psicolaboral_2020.docx")
            #     new_doc_1 = keyword_replacer_document(doc,create_replacer(data,row = nro))
            #     print("FLUJO 2")
            #     print(data[nro][column_namer])
            #     new_doc_1.save('data/output/{path_id_user}/{name}_{nro}.docx'.format(path_id_user=folder_name,name=main_name,nro=nro))
        ...
        for nro in range(0,nro_report):
            ...
            # doc = docx.Document("in_word/Informe Psicolaboral_2020.docx")
            new_doc = keyword_replacer_document(doc,create_replacer(data,row = nro))
            new_doc.save('data/output/{path_id_user}/{name}_{nro}.docx'.format(path_id_user=folder_name,name=main_name,nro=nro))
